...
| Reproducible viewport screenshots | Browser screenshots are hard to reproduce exactly across zoom state, viewport size, and local display scaling. | The interactive viewer state depends on browser layout and user actions. | `render-view` exports a fixed level-0 viewport around a requested center point with optional patch, heatmap, and annotation overlays. | Add viewer UI actions to copy the current center/window into a `render-view` command. |
