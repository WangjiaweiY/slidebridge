...
| Reproducible viewport screenshots | Browser screenshots are hard to reproduce exactly across zoom state, viewport size, and local display scaling. | The interactive viewer state depends on browser layout and user actions. | `render-view` exports a fixed level-0 viewport around a requested center point with optional patch, heatmap, and annotation overlays. The viewer can now copy the current viewport as a `render-view` command and download it as PNG. | Add browser visual tests around snapshot export. |
| Refresh-safe viewer state | Refreshing the browser could lose the selected slide, viewport, tab, and overlay filters. | Early viewer state lived only in JavaScript variables. | Viewer state is now encoded in the URL and restored on load, and the information panel can copy a reproducible viewer URL. | Add Playwright coverage for URL restore across directory mode and overlay filters. |
| Viewer visual regressions | Viewer CSS changes could reintroduce blank slides, broken sidebar scrolling, or missing controls. | The viewer was tested mostly through API and HTML string checks. | Playwright visual smoke tests now exercise synthetic slide rendering, sidebar scrolling, and URL state restore in CI. | Expand to pixel-baseline comparisons if layout stabilizes further. |
