...
`remote-view` 会在远端服务器读取 WSI，在本地通过 SSH tunnel 打开浏览器 viewer。切片不需要复制到本地，适合在工作站或服务器上调试大规模 cohort、TCGA/CPTAC/internal research slides 等场景。它使用本机 `ssh` 客户端连接远端：推荐 SSH key；如果服务器允许密码登录，也可以在终端按 SSH 提示输入密码。
